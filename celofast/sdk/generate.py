...
from celofast.sdk.model import ModelSpec, ObjectSpec, normalize

PACKAGE_FILES = ("__init__.py", "objects.py", "py.typed")
MARKER = "__celofast__"
HEADER = "# Generated by celofast km pull. Do not edit.\n"
# A qualified datetime keeps a business field named "datetime" from shadowing
# its own annotation inside the generated class body.
_TYPES = {"datetime": "_dt.datetime"}


def _variables(model: ModelSpec, capture: Capture) -> dict[str, str | None]:
    """KM inputs used by generated expressions, with their data types."""
    inputs = capture.input_variables or {}
    used = {name for spec in model.objects for field in spec.fields for name in placeholders(field.expression)}
    return {name: inputs.get(name, {}).get("dataType") for name in sorted(used)}
...
def _objects(model: ModelSpec, capture: Capture) -> str:
    ...
    lines += [
        ...
        *(f"        {name}={value!r},\n" for name, value in capture.source.model_dump().items()),
        ...
        f"    variables={_variables(model, capture)!r},\n",
        f"    objects=(\n{names}    ),\n",
        ")\n",
    ]
    return "".join(lines)
